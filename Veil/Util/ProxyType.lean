...
The main interface is the `veil_proxy_equiv%` elaborator, where `veil_proxy_equiv% t` gives an equivalence
between the proxy type for `t` and `t`. See the documentation for `veil_proxy_equiv%` for an example.

For debugging information, use `set_option trace.veil.proxyType true`.

It is possible to reconfigure the machinery to generate other types. See `ensureProxyEquiv`
and look at how it is used in `veil_proxy_equiv%`.

## Implementation notes

For inductive types with `n` constructors, the `veil_proxy_equiv%` elaborator creates a proxy
...
for each `C i`. One seems to need to add, for example, an explicit `[∀ i, Enumeration (C i)]`
instance given `∀ i, Enumeration (C i)`.

-/



namespace Veil.ProxyType
open Lean Elab Lean.Parser.Term
open Meta Command

initialize registerTraceClass `veil.proxyType
...
the `Enumeration` instances for it go through `Sigma`s anyway.
...
/-- Default configuration. Defines helpers under `Veil.Generated.ProxyType`, keyed by the
fully qualified inductive name. Uses `Unit`, `PLift`, `Sigma`, `Empty`, and `Sum`. -/
def ProxyEquivConfig.default (indVal : InductiveVal) : ProxyEquivConfig where
  proxyName := generatedName `ProxyType indVal.name `proxyType
  proxyEquivName := generatedName `ProxyType indVal.name `equiv
...
def ensureProxyEquiv (config : ProxyEquivConfig) (indVal : InductiveVal) : TermElabM Unit := do
  ...
  forallBoundedTelescope indVal.type indVal.numParams fun params _sort => do
    ...
    trace[veil.proxyType] "proxy type: {ctype}"
    ...
    if let some const := (← getEnv).find? config.proxyName then
      ...
      trace[veil.proxyType] "proxy type already exists"
    else
      addAndCompile <| Declaration.defnDecl
        { name := config.proxyName
          ...
          value := ctype' }
      -- Set to be reducible so that typeclass inference can see it's a Enumeration
      setReducibleAttribute config.proxyName
      modifyEnv (addProtected · config.proxyName)
      -- Add a docstring
      addDocStringCore config.proxyName s!"A \"proxy type\" equivalent to `{indVal.name}` that is \
        ...
        (Generated by the `veil_proxy_equiv%` elaborator.)"
      trace[veil.proxyType] "defined {config.proxyName}"

    -- Create the `Equiv`
    let equivType ← mkAppM ``Veil.Equiv #[ctype, mkAppN (mkConst indVal.name levels) params]
    if let some const := (← getEnv).find? config.proxyEquivName then
      ...
      trace[veil.proxyType] "proxy equivalence already exists"
    else
      trace[veil.proxyType] "constructing proxy equivalence"
      ...
      trace[veil.proxyType] "elaborated equivalence{indentExpr equiv}"
      ...
      addAndCompile <| Declaration.defnDecl
        { name := config.proxyEquivName
          ...
          value := equiv' }
      modifyEnv (addProtected · config.proxyEquivName)
      addDocStringCore config.proxyEquivName s!"An equivalence between the \"proxy type\" \
        ...
        (Generated by the `veil_proxy_equiv%` elaborator.)"
      trace[veil.proxyType] "defined {config.proxyEquivName}"

/-- Helper function for `veil_proxy_equiv% type : expectedType` elaborators.
...
def elabProxyEquiv (type : Term) (expectedType? : Option Expr) :
  ...
  if let some expectedType := expectedType? then
    let equivType ← Term.elabType (← `($(mkIdent ``Veil.Equiv) _ $(← Term.exprToSyntax type)))
    unless ← isDefEq expectedType equivType do
      throwError
        "Could not unify expected type{indentExpr expectedType}\nwith{indentExpr equivType}"
  let type ← Term.tryPostponeIfHasMVars type "In veil_proxy_equiv% elaborator"
  let type ← whnf type
...
/--
The term elaborator `veil_proxy_equiv% α` for a type `α` elaborates to an equivalence `β ≃ α`
...
`Veil.Generated.ProxyType.I.proxyType` and `Veil.Generated.ProxyType.I.equiv`.

The elaborator makes use of the expected type, so `(veil_proxy_equiv% _ : _ ≃ α)` works.
...
```
veil_proxy_equiv% (foo n α) : Unit ⊕ Bool ⊕ (x : Fin n) × Fin x ⊕ (_ : Bool) × α ≃ foo n α
```
-/
syntax (name := veil_proxy_equiv) "veil_proxy_equiv% " term : term

/-- Elaborator for `veil_proxy_equiv%`. -/
@[term_elab veil_proxy_equiv]
def elab_veil_proxy_equiv : Elab.Term.TermElab := fun stx expectedType? =>
  match stx with
  | `(veil_proxy_equiv% $t) => do
