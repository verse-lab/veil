...
def Module.defineAssertion (mod : Module) (base : StateAssertion) : CommandElabM Module := do
  ...
  if mod._useLocalRPropTC && !(← isModelCheckCompileMode) && (← hasVerificationSupport) then liftTermElabM do
...
def Module.defineGhostDefinition (mod : Module) (name : Name) (params : Option (TSyntax `Lean.explicitBinders)) (term : Term) (justTheory : Bool := false)
  ...
  if isRelation && mod._useLocalRPropTC && !(← isModelCheckCompileMode) && (← hasVerificationSupport) then
    liftTermElabM do
      let ref ← getRef
      if justTheory then
