...
def Module.ensureSpecIsFinalized (mod : Module) (stx : Syntax) : CommandElabM Module := do
  ...
  if let some support ← getVerificationSupport? then
    return ← support.finalizeSpec mod stx
  -- Keep assumptions available for explicit `assumptions_hold_by` proofs.
  let (assumptionsCmd, mod) ← mod.assembleAssumptions
  elabVeilCommand assumptionsCmd
  let (labelCmds, mod) ← mod.assembleLabel
  for cmd in labelCmds do elabVeilCommand cmd
