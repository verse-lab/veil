...
def VeilExecM.assert (p : Prop) [Decidable p] (ex : ExId) : VeilExecM m σ ρ Unit := do
...
lemma VeilExecM.wp_assert (p : Prop) [Decidable p] (ex : ExId) :
  wp (@VeilExecM.assert m σ ρ p _ ex) post = fun r s => if p then post () r s else hd ex := by
...
lemma VeilM.wp_assert (p : Prop) [Decidable p] (ex : ExId) :
  wp (@VeilM.assert m σ ρ p _ ex) post = fun r s => if p then post () r s else hd ex := by
  simp [VeilM.assert, MPropLift.wp_lift, wpSimp]

@[wpSimp]
lemma VeilM.wp_get [IsSubStateOf σₛ σ] :
  wp (get : VeilM m σ ρ σₛ) post = fun r s => post (getFrom s) r s := by
  simp [get, getThe, MonadStateOf.get, MPropLift.wp_lift]; ext
  erw [StateT.wp_get]

@[wpSimp]
lemma VeilM.wp_modifyGet [IsSubStateOf σₛ σ] :
  wp (modifyGet f : VeilM m σ ρ σₛ) post = fun r s => post (f (getFrom s)).1 r (setIn (f (getFrom s)).2 s) := by
...
lemma VeilExecM.wp_read [IsSubStateOf ρₛ ρ] :
  wp (read : VeilExecM m σ ρ ρₛ) post = fun r s => post (getFrom r) r s := by
