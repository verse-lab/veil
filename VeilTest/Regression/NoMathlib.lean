...
-- User declarations beside a type must not be mistaken for deriving helpers.
namespace UserNames
inductive Choice where
  | empty
  | item (b : Bool)
...
def Choice.proxyType := Nat
def Choice.proxyTypeEquiv := 42

deriving instance Enumeration for Choice
#guard (Enumeration.allValues (α := Choice)).length == 3

def firstProxy := (veil_proxy_equiv% Choice : Veil.Equiv _ Choice)
def reusedProxy := (veil_proxy_equiv% Choice : Veil.Equiv _ Choice)
example : firstProxy = reusedProxy := rfl
#guard Choice.proxyTypeEquiv == 42

inductive Color where
  | red | blue
  deriving DecidableEq

def Color.enumList := [42]
def Color.enumList_getElem?_ctorIdx_eq := 42
def Color.enumList_nodup := 42

deriving instance Enumeration, FinEncodable for Color
#guard Enumeration.allValues (α := Color) == [.red, .blue]
#guard FinEncodable.card Color == 2
#guard Color.enumList == [42]
end UserNames
