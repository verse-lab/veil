...
action Phase1a (b : ballot){
  require ¬ (∃ m, msgTset.contains m msgs ∧ m.msgType = MsgType.Phase1a ∧ m.bal = b)
...
action Phase2a (b : ballot) {
  require ¬ (∃ m, msgTset.contains m msgs ∧ m.msgType = MsgType.Phase2a ∧ m.bal = b)
  let v ← pick value
  let Q ← pick quorum
  let S :| ∀ m, msgTset.contains m S → (msgTset.contains m msgs ∧ m.msgType = MsgType.Phase1b ∧ m.bal = b)
  assume ∀ a, member a Q → ∃ m, msgTset.contains m S ∧ m.acc = a
  assume (∀ m, msgTset.contains m S → m.maxVBal = minusOne) ∨
          (∃ c, (lt c b) ∧
            (∀ m, msgTset.contains m S → tot.le m.maxVBal c) ∧
            (∃ m, msgTset.contains m S ∧ m.maxVBal = c ∧ m.val = v))
  let sentMsg : Msg acceptor value ballot := {
...
ghost relation MsgInv1b :=
  ∀ m, msgTset.contains m msgs → (m.msgType = MsgType.Phase1b →
        (tot.le m.bal (maxBal m.acc)
      ∧ (VotedForIn m.acc m.val m.maxVBal ∨ (m.maxVBal = (default : ballot) ∧ m.maxVBal = minusOne))
      ∧ (∀ c, (lt m.maxVBal c) ∧ lt c m.bal → ∀ v, ¬ VotedForIn m.acc v c)
  ))

ghost relation MsgInv2a :=
  ∀ m, msgTset.contains m msgs → (m.msgType = MsgType.Phase2a →
      (SafeAt m.val m.bal ∧
       (∀ ma, msgTset.contains ma msgs →
         (ma.msgType = MsgType.Phase2a ∧ ma.bal = m.bal → ma = m))))


ghost relation MsgInv2b :=
  ∀ m, msgTset.contains m msgs → (m.msgType = MsgType.Phase2b →
      (∃ ma, msgTset.contains ma msgs ∧
        (ma.msgType = MsgType.Phase2a ∧ ma.bal = m.bal ∧ ma.val = m.val) ∧
