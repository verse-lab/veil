...
theorem Phase1a_VotedOnce (ρ : Type) (σ : Type) (acceptor : Type) [acceptor_dec_eq : DecidableEq.{1} acceptor]
  ...
  -- Phase1a only adds Phase1a message, so Phase2b messages are preserved
  intro m1 m2 _ _ a1 a2 b' v1 v2 hc1 ht1 hv1 hb1 ha1 hc2 ht2 hv2 hb2 ha2
  -- m1 and m2 cannot be the newly added Phase1a message
  have hmne1 : m1 ≠ { msgType := MsgType.Phase1a, acc := default, val := default, bal := b, maxVBal := default } := by
    intro heq; rw [heq] at ht1; simp at ht1
  have hmne2 : m2 ≠ { msgType := MsgType.Phase1a, acc := default, val := default, bal := b, maxVBal := default } := by
