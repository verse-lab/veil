...
@[veil]
theorem Phase1a_AccInv (ρ : Type) (σ : Type) (acceptor : Type) [acceptor_dec_eq : DecidableEq.{1} acceptor]
  ...
  by
  unveil
  -- Phase1a doesn't change maxVBal, maxBal, or maxVal; only adds a Phase1a message
  intro _ _ a
  ...
  · intro x c hle hne hxcontains hxtype hxbal
    have hmne : x ≠ { msgType := MsgType.Phase1a, acc := default, val := default, bal := b, maxVBal := default } := by
      intro heq
      rw [heq] at hxtype
      simp at hxtype
