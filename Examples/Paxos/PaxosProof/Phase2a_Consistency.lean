...
@[veil]
theorem Phase2a_Consistency (ρ : Type) (σ : Type) (acceptor : Type) [acceptor_dec_eq : DecidableEq.{1} acceptor]
  ...
  by
  unveil
  -- Goal: show Consistency is preserved when Phase2a inserts a Phase2a message
  -- Key insight: Phase2a only adds a Phase2a message, but Consistency/Chosen/VotedForIn
  -- only care about Phase2b messages. So any Phase2b message in the new set
  -- was already in the old set.
  -- After unveil we have: b (ballot param) already bound
  -- Guards: b ≠ none, no 2a msg for b yet
  -- Picks: v (value), Q (quorum), selectedAcceptors (AcceptorSet)
  -- More guards for quorum coverage and value selection
  intro _ _ v Q selectedAcceptors _ _ _ v1 v2 b1 q1 hv1 b2 q2 hv2
  ...
  -- Goal 1: show ∀ a, member a q2 → VotedForIn a v2 b2 (in the old set)
  · intro a ha
    ...
    exact hcontains
  -- Goal 2: show ∀ a, member a q1 → VotedForIn a v1 b1 (in the old set)
  · intro a ha
