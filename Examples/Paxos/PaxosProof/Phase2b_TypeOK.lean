...
theorem Phase2b_TypeOK (ρ : Type) (σ : Type) (acceptor : Type) [acceptor_dec_eq : DecidableEq.{1} acceptor]
  ...
  by
  unveil
  -- Goal: TSet.contains t (filter ...) → ∀ a', le (if a = a' then t.bal else st.maxVBal a') (if a = a' then t.bal else st.maxBal a')
  intro hcontains a'
  -- Extract TypeOK from hinv
  obtain ⟨_, hTypeOK, _⟩ := hinv
  -- Case split on whether a = a'
  by_cases h : a = a'
  · -- a = a': maxVBal a = m.bal and maxBal a = m.bal, so le m.bal m.bal by reflexivity
