...
theorem Phase2a_MsgInv2b (ρ : Type) (σ : Type) (acceptor : Type) [acceptor_dec_eq : DecidableEq.{1} acceptor]
  ...
  by
  unveil
  intro hbne hno2a v _ _ _ _ _ hcontains hmsgtype
  ...
  have hmane : ma ≠ newMsg := by
    intro heq
    subst heq
    have hpred : (decide (newMsg.msgType = MsgType.Phase2a) && decide (newMsg.bal = b)) = true := by
      simp only [decide_eq_true_iff, Bool.and_eq_true]
      exact ⟨rfl, rfl⟩
    have hma_in_filter : TSet.contains newMsg (TSet.filter st.msgs (fun msg => decide (msg.msgType = MsgType.Phase2a) && decide (msg.bal = b))) = true := by
      exact TSet.contains_filter_of newMsg st.msgs _ hma_contains hpred
    have hfalse := TSet.count_zero_not_contains newMsg _ hno2a
    rw [hma_in_filter] at hfalse
    simp at hfalse
