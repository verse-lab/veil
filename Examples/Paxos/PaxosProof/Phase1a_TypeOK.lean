...
@[veil]
theorem Phase1a_TypeOK (ρ : Type) (σ : Type) (acceptor : Type) [acceptor_dec_eq : DecidableEq.{1} acceptor]
  ...
  by
  unveil
  -- Phase1a doesn't modify maxBal or maxVBal, so TypeOK is preserved
  -- TypeOK: ∀ a, tot.le (maxVBal a) (maxBal a)
  -- Introduce the guards from the Phase1a action
  intro _ _ a
  -- From hinv we have the pre-state TypeOK
