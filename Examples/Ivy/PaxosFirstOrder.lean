...
action join_round (n : node) (r : round) {
  ...
  require r ≠ none
  require one_a r
  require ¬ (∃ (r' : round) (rMAX : round) (v : value),
    one_b_max_vote n r' rMAX v ∧ ¬ tot.le r' r)
  let maxr : round ← pick
  let v : value ← pick
  -- Find the maximal vote in a round less than r.
  require
    (maxr = none ∧
        ∀ (mAXR : round) (v' : value), ¬ (¬ tot.le r mAXR ∧ vote n mAXR v')) ∨
      (maxr ≠ none ∧ ¬ tot.le r maxr ∧ vote n maxr v ∧
        (∀ (mAXR : round) (v' : value),
          (¬ tot.le r mAXR ∧ vote n mAXR v') → tot.le mAXR maxr))
  -- Send the 1b message.
  ...
  require
    (maxr = none ∧
      ...
      (maxr ≠ none ∧
        ...
        (∀ (n : node) (mAXR : round) (v : value),
          (member n q ∧ one_b_max_vote n r mAXR v ∧ mAXR ≠ none) → tot.le mAXR maxr))
  -- Propose value v.
  proposal r v := true
}
...
invariant [decision_from_quorum]
  (∀ (R : round) (V : value),
    (∃ (N : node), decision N R V) →
      ∃ (Q : quorum), ∀ (N : node), member N Q → vote N R V)

invariant [one_b_max_vote_properties1]
  one_b_max_vote N R2 none V1 ∧ ¬ tot.le R2 R1 → ¬ vote N R1 V2

invariant [one_b_max_vote_properties2]
  one_b_max_vote N R RMAX V ∧ RMAX ≠ none → ¬ tot.le R RMAX ∧ vote N RMAX V

invariant [one_b_max_vote_properties3]
  one_b_max_vote N R RMAX V ∧ RMAX ≠ none ∧ ¬ tot.le R ROTHER ∧ ¬ tot.le ROTHER RMAX →
    ¬ vote N ROTHER VOTHER

invariant [none_vote] ¬ vote N none V

invariant [choosable_proposal]
  (∀ (R1 : round) (R2 : round) (V1 : value) (V2 : value) (Q : quorum),
    ¬ tot.le R2 R1 ∧ proposal R2 V2 ∧ V1 ≠ V2 →
      ∃ (N : node) (R3 : round) (RMAX : round) (V : value),
        member N Q ∧ ¬ tot.le R3 R1 ∧ one_b_max_vote N R3 RMAX V ∧ ¬ vote N R1 V1)
