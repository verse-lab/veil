...
`RingConc.lean` (module `RingConc`) is a *concrete* ring–leader–election
...
`RingAbs.lean` (module `RingAbs`) is the *abstract* version of the same protocol,
...
open scoped RingConc RingAbs

namespace RingRef

/-! ## Characterizations of the concrete transition system -/

/-- Abbreviation for the concrete (`RingConc`) transition system. -/
noncomputable abbrev CC : RelationalTransitionSystem RingConc.Theory
    (RingConc.State RingConc.FieldAbstractType) RingConc.Label :=
  RingConc.relationalTransitionSystem

theorem cc_assumptions (th : RingConc.Theory) :
    CC.assumptions th ↔ th.allNodes.Nodup ∧ 1 < th.allNodes.length := by
  simp only [CC, RingConc.relationalTransitionSystem, invSimp, RingConc.Assumptions]
  rfl

theorem cc_init (th : RingConc.Theory) (s : RingConc.State RingConc.FieldAbstractType) :
    CC.init th s ↔ (⟨[], []⟩ : RingConc.State RingConc.FieldAbstractType) = s := by
  simp only [CC, RingConc.relationalTransitionSystem, nextSimp, RingConc.Init,
    CanonicalField.set]
  rfl

theorem cc_send (th : RingConc.Theory) (s s' : RingConc.State RingConc.FieldAbstractType) :
    CC.tr th s RingConc.Label.send s' ↔
    ∃ n ∈ th.allNodes,
      if (⟨n, n, RingConc.nextNode n th⟩ : RingConc.Message) ∉ s.messages then
        (⟨s.leader, s.messages.insertOrdered ⟨n, n, RingConc.nextNode n th⟩⟩ :
          RingConc.State RingConc.FieldAbstractType) = s'
      else s = s' := by
  simp only [CC, RingConc.relationalTransitionSystem, nextSimp, RingConc.NextAct,
    RingConc.FieldAbstractType, CanonicalField.set]

theorem cc_recv (th : RingConc.Theory) (s s' : RingConc.State RingConc.FieldAbstractType) :
    CC.tr th s RingConc.Label.recv s' ↔
    ∃ m ∈ s.messages,
      if m.payload = m.dst ∧ m.dst ∉ s.leader then
        (⟨m.dst :: s.leader, s.messages.erase m⟩ :
          RingConc.State RingConc.FieldAbstractType) = s'
      else if m.dst ≤ m.payload then
        if (⟨m.payload, m.dst, RingConc.nextNode m.dst th⟩ : RingConc.Message) ∉
            s.messages.erase m then
          (⟨s.leader, (s.messages.erase m).insertOrdered
              ⟨m.payload, m.dst, RingConc.nextNode m.dst th⟩⟩ :
            RingConc.State RingConc.FieldAbstractType) = s'
        else (⟨s.leader, s.messages.erase m⟩ : RingConc.State RingConc.FieldAbstractType) = s'
      else (⟨s.leader, s.messages.erase m⟩ : RingConc.State RingConc.FieldAbstractType) = s' := by
  simp only [CC, RingConc.relationalTransitionSystem, nextSimp, RingConc.NextAct,
    RingConc.FieldAbstractType, CanonicalField.set]
...
/-- Abbreviation for the abstract (`RingAbs`) transition system over `node`. -/
noncomputable abbrev CA : RelationalTransitionSystem (RingAbs.Theory node)
    (RingAbs.State (RingAbs.FieldAbstractType node)) (RingAbs.Label node) :=
  RingAbs.relationalTransitionSystem node

omit [DecidableEq node] in
theorem ca_init (th : RingAbs.Theory node) (s : RingAbs.State (RingAbs.FieldAbstractType node)) :
    (CA (node := node)).init th s ↔
      (⟨fun _ => false, fun _ _ => false⟩ : RingAbs.State (RingAbs.FieldAbstractType node)) = s := by
  simp only [CA, RingAbs.relationalTransitionSystem, nextSimp, RingAbs.Init,
    CanonicalField.set]
  rfl

theorem ca_send (th : RingAbs.Theory node) (s s' : RingAbs.State (RingAbs.FieldAbstractType node))
    (a b : node) :
    (CA (node := node)).tr th s (RingAbs.Label.send a b) s' ↔
      (∀ Z : node, a ≠ b ∧ (Z ≠ a ∧ Z ≠ b → Between.btw a b Z)) ∧
      (⟨s.leader, fun x y => if a = x ∧ b = y then true else s.pending x y⟩ :
        RingAbs.State (RingAbs.FieldAbstractType node)) = s' := by
  simp only [CA, RingAbs.relationalTransitionSystem, nextSimp, RingAbs.NextAct,
    RingAbs.instAbstractFieldRepresentation, canonicalFieldRepresentation,
    RingAbs.FieldAbstractType, RingAbs.State.Label.toDomain,
    RingAbs.State.Label.toCodomain, CanonicalField.set]

theorem ca_recv (th : RingAbs.Theory node) (s s' : RingAbs.State (RingAbs.FieldAbstractType node))
    (sender a b : node) :
    (CA (node := node)).tr th s (RingAbs.Label.recv sender a b) s' ↔
      (∀ Z : node, a ≠ b ∧ (Z ≠ a ∧ Z ≠ b → Between.btw a b Z)) ∧
      s.pending sender a = true ∧
      (if sender = a then
          (⟨fun x => if a = x then true else s.leader x,
            fun x y => if sender = x ∧ a = y then false else s.pending x y⟩ :
            RingAbs.State (RingAbs.FieldAbstractType node)) = s'
...
theorem nextNode_eq_next {th : RingConc.Theory} {n : ℕ} (hn : n ∈ th.allNodes) :
    RingConc.nextNode n th = th.allNodes.next n hn := by
  simp only [RingConc.nextNode, instIsSubReaderOfRefl.readFrom_id]
  exact dif_pos hn
...
theorem isNext_nextNode {th : RingConc.Theory} (hnodup : th.allNodes.Nodup)
    (hlen : 1 < th.allNodes.length) (n : ℕ) (hn : n ∈ th.allNodes)
    (hnn : RingConc.nextNode n th ∈ th.allNodes) (Z : RNode th.allNodes) :
    (⟨n, hn⟩ : RNode th.allNodes) ≠ ⟨RingConc.nextNode n th, hnn⟩ ∧
      (Z ≠ ⟨n, hn⟩ ∧ Z ≠ ⟨RingConc.nextNode n th, hnn⟩ →
        Between.btw (⟨n, hn⟩ : RNode th.allNodes) ⟨RingConc.nextNode n th, hnn⟩ Z) := by
  have heq : (⟨RingConc.nextNode n th, hnn⟩ : RNode th.allNodes)
...
theorem nextNode_inj {th : RingConc.Theory} (hnodup : th.allNodes.Nodup)
    {x y : ℕ} (hx : x ∈ th.allNodes) (hy : y ∈ th.allNodes)
    (h : RingConc.nextNode x th = RingConc.nextNode y th) : x = y := by
...
-/
def rel (th : RingConc.Theory)
    (sc : RingConc.State RingConc.FieldAbstractType)
    (sa : RingAbs.State (RingAbs.FieldAbstractType (RNode th.allNodes))) : Prop :=
  ...
  (∀ m ∈ sc.messages, m.src ∈ th.allNodes ∧ m.payload ∈ th.allNodes ∧
      m.dst = RingConc.nextNode m.src th)

/-- Under well-formedness, a message is determined by its `(payload, dst)` pair. -/
theorem msg_unique {th : RingConc.Theory} (hnodup : th.allNodes.Nodup)
    {ms : List RingConc.Message}
    (hwf : ∀ m ∈ ms, m.src ∈ th.allNodes ∧ m.payload ∈ th.allNodes ∧
      m.dst = RingConc.nextNode m.src th)
    {m1 m2 : RingConc.Message} (h1 : m1 ∈ ms) (h2 : m2 ∈ ms)
...
theorem perm_insertOrdered {α : Type} [Ord α] (x : α) (l : List α) :
    List.Perm (l.insertOrdered x) (x :: l) := by
  induction l with
  | nil => exact .refl _
  | cons a l ih =>
    change List.Perm (if compare x a == .lt then x :: a :: l
      else a :: l.insertOrdered x) (x :: a :: l)
    split
    · exact .refl _
    · exact (ih.cons a).trans (.swap ..)

theorem mem_insertOrdered {α : Type} [Ord α] (x y : α) (l : List α) :
    y ∈ l.insertOrdered x ↔ y = x ∨ y ∈ l := by
  exact (perm_insertOrdered x l).mem_iff.trans List.mem_cons

theorem nodup_insertOrdered {α : Type} [Ord α] (x : α) (l : List α) :
    (l.insertOrdered x).Nodup ↔ (x :: l).Nodup := by
  exact (perm_insertOrdered x l).nodup_iff

/-! ## The CSLib simulation -/

theorem ca_assumptions_true {node : Type} [DecidableEq node] [Inhabited node]
    [TotalOrder node] [Between node] (th : RingAbs.Theory node) :
...
theorem initially_related (th : RingConc.Theory) [NeZero th.allNodes.length]
    (sc : RingConc.State RingConc.FieldAbstractType) (hinit : CC.init th sc) :
    ∃ sa, (CA (node := RNode th.allNodes)).init ⟨⟩ sa ∧ rel th sc sa := by
  rw [cc_init] at hinit
  subst hinit
...
theorem sim (th : RingConc.Theory) (hnodup : th.allNodes.Nodup)
  ...
  | send =>
    rw [cc_send] at htr
    obtain ⟨n, hn, htr⟩ := htr
    have hnn : RingConc.nextNode n th ∈ th.allNodes := by
      rw [nextNode_eq_next hn]; exact List.next_mem _ _ _
    set nodeN : RNode th.allNodes := ⟨n, hn⟩ with hnodeN
    set nodeNN : RNode th.allNodes := ⟨RingConc.nextNode n th, hnn⟩ with hnodeNN
    set msg : RingConc.Message := ⟨n, n, RingConc.nextNode n th⟩ with hmsg
    split at htr
  ...
  | recv =>
    ...
    have hnnd : RingConc.nextNode m.dst th ∈ th.allNodes := by
    ...
    set b : RNode th.allNodes := ⟨RingConc.nextNode m.dst th, hnnd⟩ with hb
    set msg2 : RingConc.Message := ⟨m.payload, m.dst, RingConc.nextNode m.dst th⟩ with hmsg2
    ...
    · rename_i hcondA_neg
      ...
      · -- receiver does not elect itself but forwards (`m.dst ≤ m.payload`).
        ...
        have hwf_common : ∀ m'' ∈ sc'.messages,
            ...
            m''.dst = RingConc.nextNode m''.src th := by
          intro m'' hm''
          rcases (hsc'M m'').mp hm'' with rfl | h
...
theorem single_leader_holds :
  ...
  have habs : ∀ sa, (CA (node := RNode th.allNodes)).reachable
      (⟨⟩ : RingAbs.Theory (RNode th.allNodes)) sa →
      (∀ N M : RNode th.allNodes, sa.leader N = true ∧ sa.leader M = true → N = M) := by
    intro sa hr
    exact RingAbs.single_leader.is_inv (node := RNode th.allNodes) (⟨⟩) sa hr
...
theorem single_leader_safety :
    CC.isInvariant (fun th st =>
      RingConc.single_leader
        (ρ := RingConc.Theory) (σ := RingConc.State RingConc.FieldAbstractType)
        (χ := RingConc.FieldAbstractType)
        (χ_rep := RingConc.instAbstractFieldRepresentation)
        (χ_rep_lawful := RingConc.instLawfulAbstractFieldRepresentation) th st) :=
  single_leader_holds
