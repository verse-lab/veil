...
#reduce ⌞? State.Label.toDomain ⌟
#reduce IteratedProd ((⌞? State.Label.toDomain ⌟) State.Label.useless)
...
instance : (rd : TheoryConcrete) → (st : StateConcrete)
    → Decidable ((fun ρ σ => single_leader ρ σ) rd st) := by
  intro rd st
...
def modelCheckerResult' := (runModelCheckerx initVeilMultiExecM nextVeilMultiExecM labelList (fun ρ σ => single_leader ρ σ) ((fun ρ σ => true)) {} id).snd
