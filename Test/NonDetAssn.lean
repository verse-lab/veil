...
info: def Test.nondet_individual : {node : Type} →
  [node_dec : DecidableEq node] → [node_ne : Nonempty node] → Veil.Wp Veil.Mode.internal (State node) PUnit :=
...
info: def Test.quantify_fresh : {node : Type} →
  [node_dec : DecidableEq node] → [node_ne : Nonempty node] → node → Veil.Wp Veil.Mode.internal (State node) PUnit :=
